...
# --- HAUPTSYSTEM (Nach erfolgreichem Login) ---
else:
    # Willkommens-Nachricht mit Uhrzeit
    aktuelle_uhrzeit = time.strftime('%H:%M')
    st.toast(f"Willkommen zurück, Silas! Login um {aktuelle_uhrzeit} Uhr bestätigt.", icon="👤")
    ...
    if menu == "System-Status":
        st.title(f"Guten Tag, Silas!")
        st.write(f"System-Statusbericht vom {time.strftime('%d.%m.2025')} | {aktuelle_uhrzeit} Uhr")
        ...
        st.info("Alle Systeme laufen im grünen Bereich. Die Sidebar ist nun dauerhaft fixiert.")
        ...
        st.code(f"[{time.strftime('%H:%M:%S')}] Willkommens-Modul v9.1 geladen.\n[STATUS] System verschlüsselt.\n[USER] Master-Login Silas.")
